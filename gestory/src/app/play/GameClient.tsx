...
// TYPES
// =====================================================
type GameState = "HUNTING" | "EXPLODING" | "QUESTION" | "FEEDBACK" | "FINISHED" | "LOADING";
...
export default function GamePage() {
  ...
  const handleSubmitScore = async () => {
    ...
    try {
      const questionsAnswered = BANK_SOAL.length;
      const correctAnswers = score / 100;
      const accuracy = questionsAnswered > 0 ? (correctAnswers / questionsAnswered) * 100 : 0;
      
      const response = await gameService.submitGameScore({
      ...
      if (response.success) {
        ...
        alert("Gagal mengirimkan skor: " + (response.message || "Unknown error"));
      }
    } catch (err) {
      console.error("Failed to submit score:", err);
      alert("Terjadi kesalahan saat mengirimkan skor.");
    } finally {
  ...
  // Loading state - sebelum game mulai
  if (questionsLoading && !BANK_SOAL.length) {
    return (
      <div className="relative h-screen w-screen overflow-hidden bg-slate-900 text-white font-sans flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-4 border-blue-500/30 border-t-blue-500 rounded-full animate-spin"></div>
          <p className="text-lg font-semibold">Loading game questions...</p>
          <p className="text-sm text-slate-400">Initializing MediaPipe...</p>
        </div>
      </div>
    );
  }

  // Error state - jika API error tapi masih bisa main dengan default
  if (questionsError && BANK_SOAL.length === DEFAULT_BANK_SOAL.length) {
    console.warn("API Error loading questions, using default:", questionsError);
  }
      ...
      {gameState === "HUNTING" && (
        ...
        >
          <div className="relative flex flex-col items-center">
            ...
            <div className="relative w-45 h-45 z-10 hover:scale-110 transition-transform duration-300 animate-bounce cursor-pointer flex items-center justify-center">
              {/* Outer Ring */}
              ...
              {/* Crosshairs - Vertical */}
              <div className="absolute -top-3.75 left-1/2 w-1.5 h-12 -translate-x-1/2 bg-white shadow-[0_0_10px_white] rounded-full z-20" />
              <div className="absolute -bottom-3.75 left-1/2 w-1.5 h-12 -translate-x-1/2 bg-white shadow-[0_0_10px_white] rounded-full z-20" />
              {/* Crosshairs - Horizontal */}
              <div className="absolute -left-3.75 top-1/2 h-1.5 w-12 -translate-y-1/2 bg-white shadow-[0_0_10px_white] rounded-full z-20" />
              <div className="absolute -right-3.75 top-1/2 h-1.5 w-12 -translate-y-1/2 bg-white shadow-[0_0_10px_white] rounded-full z-20" />
            </div>
      ...
      {gameState === "FINISHED" && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-slate-900/95 backdrop-blur-md cursor-auto">
          <div className="max-w-2xl w-full bg-white rounded-[48px] p-12 shadow-2xl text-slate-900 text-center">
            ...
            <div className="bg-linear-to-br from-blue-500 to-blue-700 text-white rounded-3xl p-8 mb-8 shadow-lg shadow-blue-200">
            ...
            {/* Input Nama & Kirim Skor ke Leaderboard */}
            <div className="mb-8 max-w-md mx-auto p-5 bg-slate-50 rounded-3xl border-2 border-slate-100">
              {scoreSubmitted ? (
                <p className="text-green-600 font-black flex items-center justify-center gap-2 text-base py-2">
                  ✅ Skor berhasil dikirim ke Leaderboard!
                </p>
              ) : (
                <div className="flex flex-col gap-3">
                  <p className="text-slate-600 text-xs font-black uppercase tracking-wider text-left pl-1">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-3.5 border-2 border-slate-200 rounded-2xl text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                    />
                    <button
                      onClick={handleSubmitScore}
                      disabled={isSubmittingScore || !playerName.trim()}
                      className="bg-blue-600 text-white px-6 py-3.5 rounded-2xl font-black hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm flex items-center justify-center min-w-[120px] transition-all"
                    >
                      {isSubmittingScore ? "Mengirim..." : "Kirim Skor"}
                    </button>
                  </div>
                </div>
              )}
